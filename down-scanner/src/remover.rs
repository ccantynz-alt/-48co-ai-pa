...
fn kill_process(pid: u32, name: &str, count: &mut usize) {
    #[cfg(windows)]
    {
        ...
        match result {
            Ok(output) if output.status.success() => {
                println!("  {} Killed: {} (PID {})", "[\u{2713}]".green().bold(), name, pid);
                *count += 1;
            }
            _ => {
                println!(
    ...
    {
        println!(
            "  {} Would kill PID {} \u{2014} {} (Windows only)",
            "[~]".yellow(),
            pid,
            name
        );
...
fn uninstall_program(uninstall_string: &str, name: &str, count: &mut usize) {
    if uninstall_string.is_empty() {
        println!(
            "  {} No uninstaller found for '{}' \u{2014} will delete files instead",
            "[~]".yellow(),
            name
        );
        return;
    ...
    #[cfg(windows)]
    {
        ...
        if uninstall_lower.contains("msiexec") {
            ...
            if let Some(guid_start) = uninstall_string.find('{') {
                if let Some(guid_end) = uninstall_string.find('}') {
                    ...
                    match result {
                        Ok(output) if output.status.success() => {
                            println!("  {} Uninstalled (MSI): {}", "[\u{2713}]".green().bold(), name);
                            *count += 1;
                            success = true;
                        }
                        _ => {}
        ...
        if !success {
            ...
            for flag in &silent_flags {
                ...
                match result {
                    Ok(output) if output.status.success() => {
                        println!("  {} Uninstalled: {}", "[\u{2713}]".green().bold(), name);
                        *count += 1;
                        success = true;
                        break;
                    }
    ...
    #[cfg(windows)]
    {
        ...
        match hive.open_subkey_with_flags(subkey, KEY_WRITE) {
            Ok(key) => match key.delete_value(value_name) {
                ...
                Err(e) => {
                    println!("  {} Failed to remove '{}': {}", "[\u{2717}]".red(), value_name, e);
                }
            },
            Err(e) => {
                println!(
...
fn delete_file(path: &str, count: &mut usize) {
    ...
    match result {
        Ok(_) => {
            println!("  {} Deleted: {}", "[\u{2713}]".green().bold(), path);
            *count += 1;
        }
        Err(e) => {
            println!("  {} Failed to delete '{}': {}", "[\u{2717}]".red(), path, e);
        }
    }
}
...
fn reset_proxy_settings(count: &mut usize) {
    #[cfg(windows)]
    {
        ...
        match hkcu.open_subkey_with_flags(key_path, KEY_WRITE) {
            ...
            Err(e) => {
                println!("  {} Failed to reset proxy: {}", "[\u{2717}]".red(), e);
            }
        }
    }
    #[cfg(not(windows))]
