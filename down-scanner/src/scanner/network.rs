...
fn check_proxy_hijack(threats: &mut Vec<Threat>) {
    ...
    if proxy_enable == 0 {
        return; // Proxy not enabled \u{2014} clean
    }
...
fn check_hosts_file(threats: &mut Vec<Threat>) {
    ...
    let content = match fs::read_to_string(hosts_path) {
        Ok(c) => c,
        Err(_) => return, // Can't read hosts file \u{2014} might need admin
    };
    ...
    for line in content.lines() {
        ...
        for hostname in &hostnames {
            ...
            if !is_legitimate {
                ...
                if is_redirect {
                    // Redirecting to a non-local IP \u{2014} very suspicious
        ...
        for (bad_prefix, description) in KNOWN_BAD_IP_PREFIXES {
            if ip.starts_with(bad_prefix) {
                threats.push(Threat {
                    ...
                    description: format!(
                        "Hosts entry '{}' redirects to suspicious IP {} \u{2014} {}",
                        hostnames.join(", "),
                        ip,
                        description
                    ),
            ...
            for hostname in &hostnames {
                if hostname.to_lowercase().contains(bad_domain) {
                    threats.push(Threat {
                        ...
                        location: hosts_path.to_string_lossy().to_string(),
                        description: format!(
                            "Hosts file references known malicious domain pattern '{}' \u{2014} {}",
        ...
        for (bad_prefix, description) in KNOWN_BAD_IP_PREFIXES {
            if remote.starts_with(bad_prefix) {
                threats.push(Threat {
                    ...
                    description: format!(
                        "Active network connection to known suspicious IP range \u{2014} {}",
                        description
                    ),
                    action: ThreatAction::ManualReview,
                });
                ...
                for (bad_dns, description) in KNOWN_BAD_DNS {
                    if ip.starts_with(bad_dns) {
                        threats.push(Threat {
                            ...
                            description: format!(
                                "DNS server {} is known malicious \u{2014} {}. \
