        ...
        for value_result in key.enum_values() {
            ...
            for bad_name in KNOWN_BAD_PROCESSES {
                if value_lower.contains(bad_name) || value_name.to_lowercase().contains(bad_name) {
                    threats.push(Threat {
                        ...
                        description: format!("Registry startup entry '{}' matches known threat '{}'. Value: {}", value_name, bad_name, value_str),
                        action: ThreatAction::RemoveStartupEntry { key_path: full_key.clone(), value_name: value_name.clone() },
                    });
                    break;
                }
            }
            ...
            for fragment in SUSPICIOUS_PATH_FRAGMENTS {
                if value_lower.contains(fragment) {
                    threats.push(Threat {
                        ...
                        category: ThreatCategory::SuspiciousStartup,
                        location: full_key.clone(),
                        description: format!("Startup entry '{}' points to suspicious location containing '{}'. Value: {}", value_name, fragment, value_str),
                        action: ThreatAction::RemoveStartupEntry { key_path: full_key.clone(), value_name: value_name.clone() },
                    });
                    break;
                }
            ...
            if !exe_path.is_empty() && !Path::new(&exe_path).exists() {
                threats.push(Threat {
                    ...
                    category: ThreatCategory::SuspiciousStartup,
                    location: full_key.clone(),
                    description: format!("Startup entry '{}' points to non-existent file: {}", value_name, exe_path),
                    action: ThreatAction::RemoveStartupEntry { key_path: full_key.clone(), value_name: value_name.clone() },
                });
            }
        }
            ...
            for bad_name in KNOWN_BAD_PROCESSES {
                if file_name.contains(bad_name) {
                    threats.push(Threat {
                        ...
                        description: format!("Startup folder contains file matching known threat '{}'", bad_name),
            ...
            if suspicious_extensions.iter().any(|ext| file_name.ends_with(ext)) {
                ...
                if !already_flagged {
                    threats.push(Threat {
                        ...
                        category: ThreatCategory::SuspiciousStartup,
                        location: file_path.clone(),
                        description: "Executable file in startup folder will run automatically on login. Verify this is intended.".to_string(),
...
fn get_startup_folder_paths() -> Vec<String> {
    ...
    if cfg!(windows) {
        if let Some(appdata) = dirs::config_dir() {
            paths.push(appdata.join("Microsoft").join("Windows").join("Start Menu").join("Programs").join("Startup").to_string_lossy().to_string());
        }
        paths.push(r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Startup".to_string());
    }
    paths
}
